...
@RestController
@RequestMapping("/auth")
// Frontend (React) URL yahan check kar lena, agar change ho toh update kar dena
@CrossOrigin(origins = "http://localhost:5173", allowCredentials = "true")
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginDto loginDto) {
        ...
        try {
            ...
            // 2. Database se user fetch karna taaki role mil sake
            UserEntity user = userRepository.findByEmail(authenticatedUser.getUsername())
                    .orElseThrow(() -> new UsernameNotFoundException("User not found"));
            ...
            // 5. MODIFICATION: Token ke saath User details (Email & Role) return karna
            Map<String, Object> response = new HashMap<>();
            response.put("token", token);
            response.put("email", user.getEmail());
            response.put("role", user.getRole()); // Ye Frontend Sidebar filter ke liye bohot zaroori hai
            response.put("message", "Login success");
